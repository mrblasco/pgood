...
variation in the submission rates across solicitation treatments
(ranging between 2 and 7 percent). We check to see whether there is an
association between the variation in submission rates and the
solicitation treatments. A Fisher's exact test fails to reject the null
of independence (p=0.026), thus indicating a statistically significant
association. We find thus evidence suggesting that the observed
variation in submission rates is attributable to our experimental
intervention.
...
reveal that: (\emph{i}) the PRIZE solicitation treatment generates 1.4,
1.6, and 3.2 times higher participation rates than the WPLACE, PCARE,
and FUND, respectively; (\emph{ii}) the PCARE and WPLACE solicitation
treatments generate basically identical participation rates; and
(\emph{iii}) the FUND solicitation treatment generates about less than
half of the participation rates of the WPLACE and PCARE solicitation
treatments.
...
\emph{Sorting.} We now look at how the propensity to participate in the
contest is associated with relevant individual factors, like the gender,
profession, and status inside the organization of the solicited worker.

Following the literature on gender-based differences in attitudes
towards competitive settings \citep{croson2009gender}, one may expect
gender to be a factor driving participation in the contest, with men
relatively more willing to sort into the competition
\citep{niederle2007women}. Contrary to this expectation, we find (Figure
\ref{fig: sorting}) that the submission rate is higher for women than
for men and testing the difference in a regression (Table
\ref{participation ols}) with controls for the other baseline
characteristics gives insignificant results. Similarly, one may expect
the presence of effects associated with differences in income and
education. In particular, it is interesting to check if there is a lower
participation of doctors relative to other staff members, given that
these are the highest paid staff members in this environment. We find
(Figure \ref{fig: sorting}, panel B) that the submission rate for
physicians is lower relative to the residual category of other workers,
whereas it is higher for nurses. Although these differences are
consistent with sorting, testing them in a regression (Table
\ref{participation ols}) with controls for the other baseline
characteristics gives again insignificant results. Finally, and somewhat
surprisingly, we find evidence of a positive effect of having an office
location, as opposed to being assigned to a ward (Figure
\ref{fig: sorting}, panel C). This effect is statistically significant
in a regression (Table \ref{participation ols}) controlling for the
gender and profession of the solicited employee.
...
\emph{Participation dynamics.} We now turn to examining participation
dynamics (Figure \ref{dynamics}). Though our data may not allow for a
complete analysis of participation dynamics, looking at the overall
submission patterns can be useful for the following reason. If employees
assigned to different solicitation treatments were sharing (either
face-to-face or electronically) the content of their solicitation with
others, one should expect participation rates to converge over time,
yielding estimates of the causal effects of a solicitation treatment
biased towards zero. Contrary to these expectations, we find no evidence
of convergence. Submissions in the PRIZE solicitation treatment are
...
\emph{Heterogeneous treatment effects.} Inspection of the participation
rates conditional to the employee's gender or profession (Figure
\ref{interactions}) suggests relevant treatment interactions based on
the gender.\footnote{We find no significant differences for interactions
  with office location, which we do not report for space limitation.}
Gender interactions may occur as a result of three main factors:
...
Similarly, one may expect treatment interactions associated with the
employee's profession to occur because, for example, the prize
opportunity could be relatively less effective for employees with a
higher income, such as doctors.
