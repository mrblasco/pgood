...
## @knitr methods_common_files

source("R/table_render.R") 
source("R/util.R")

## @knitr figure_timeline
...
design <- function() {
p <- c("win an Apple iPad mini"
	, "win project funding up to $20,000 to turn your ideas into actions"
	, "improve patient care at the Heart Center"
	, "improve the workplace at the Heart Center")
	...
	m <- cbind(m, `Submit your ideas to`=c(as.character(txt$paragraph), NA))
	return(m)
}
add <- rep()
add$cmd <- "& \\multicolumn{2}{c}{\\emph{Employees:}}& \\multicolumn{1}{c}{\\emph{Randomized solicitation:}}\\\\\n \\cmidrule(lr){2-3}"
add$pos <- -1
table_render(design(), caption="Experimental design", label="experimental-design"
